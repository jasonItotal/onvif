...
new Cam({
	...
	function read_and_process_keyboard() {
		...
		process.stdin.on('keypress', function(ch, key) {
			...
			if      (key && key.name == 'up')    {move(0,1,0,'up');} else if (key && key.name == 'down')  {move(0,-1,0,'down');} else if (key && key.name == 'left')  {move(-1,0,0,'left');} else if (key && key.name == 'right') {move(1,0,0,'right');} else if (ch  && ch       == '-')     {move(0,0,-1,'zoom out');} else if (ch  && ch       == '+')     {move(0,0,1,'zoom in');}
			// On English keyboards '+' is "Shift and = key"
			// Accept the "=" key as zoom in
			else if (ch  && ch       == '=')     {move(0,0,1,'zoom in');} else if (ch  && ch >= '1' && ch <= '9') {goto_preset(ch);}
		});
	}
